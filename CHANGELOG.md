...
- **Cursor `.cursor/rules/*.mdc` rule files are now scanned.** Cursor's modern
  rules format (markdown-with-frontmatter) is treated as AI-config markdown, so
  the existing credential, prompt-injection, and exfiltration rules apply to it.
  Previously the `.mdc` extension was unrecognized and these files were skipped
